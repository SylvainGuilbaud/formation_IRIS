...
def main():
  ...
  for index, row in filtered_users.iterrows():
      ...
      try:
          if irispy.classMethodBoolean('Security.Users','Exists',username)==0:
            status = irispy.classMethodString('Security.Users','Create',username,'connectBNA','password','','USER','','',1,1)
